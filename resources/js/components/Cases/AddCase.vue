    ...
    <div>
        ...
        <div>
            <div v-if="currentStep==1">
                ...
                <div class="form-group">
                    <label for="location" class="form-label">How Many Beneficaries</label>
                    <select v-model="beneficiariesCount" @change="initForm(beneficiariesCount)" name="location" id="location" class="form-control">
        ...
        <button v-if="currentStep > 1" type="button" @click="back" class="btn btn-primary">Back</button>
        <button v-if="currentStep < totalSteps" type="button" @click="next" class="btn btn-primary">Next</button>
        <button v-if="currentStep == totalSteps && currentStep > 1" type="submit" class="btn btn-success">Create</button>
    </div>
</template>
<script>
...
export default {
	...
	methods: {
        initForm (beneficiariesCount){
            console.log('hit');
            let diff  = beneficiariesCount - this.beneficiaries.length;
            if(diff > 0){
                for (let i = 0; i < diff; i++) {
                    this.addNewBeneficiaryForm();
                }
            }else{
                diff = diff * (-1);
                for (let i = 0; i < diff; i++) {
                    this.beneficiaries.pop();
                }
            }
            this.totalSteps = this.beneficiaries.length + 1;
        },
