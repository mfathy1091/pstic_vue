	...
	<div class="p-0">
        <div class="card-body">
            <div class="form-inline mr-2 ml-2 mt-3">
                ...
                <button class="btn btn-secondary btn-sm mr-2" @click="getStats">
					Execute <i class="fas fa-bolt ml-2"></i>
				</button>

                <select v-model="filter.user_id" @change="getStats" class="custom-select my-1 mr-sm-2" id="inlineFormCustomSelectPref">
					<option value='-1' disabled>Worker...</option>
					<option v-for='user in users' :value='user.id' :key="user.id">{{ user.full_name }}</option>
                </select>

                <select v-model="filter.status_id" @change="getStats" class="custom-select m-1" id="inlineFormCustomSelectPref">
