	...
	<div class="p-0">
        ...
        <div class="card mb-0">
            ...
            <div class="row mt-3 table-responsive m-0">
                <p v-show="!pswBeneficiaries.length" class="font-italic ml-5">You have no Cases!</p>

                <table v-show="pswBeneficiaries.length" class="border table table-hover table-sm">
                    <thead>
                        <tr>
                            <th>Case Number</th>
                    ...
                    <tbody v-if="pswBeneficiaries">
                        <tr v-for="referral in this.pswBeneficiaries" :key="referral.id">
                            ...
                            <td>
                                <router-link
                                :to="{name: 'referralDetails', params: { caseeId: referral.casee_id, referralId: referral.id }}"
...
export default {
	...
	methods: {
        ...
        getPswBeneficiaries(){
			this.$Progress.start();
			axios.get('/api/referral-beneficiaries', { params: { is_new: this.filter.is_new, status_id: this.filter.status_id, month_id: this.filter.month.id, user_id: this.filter.user_id} })
			.then((response) => {
				// success
